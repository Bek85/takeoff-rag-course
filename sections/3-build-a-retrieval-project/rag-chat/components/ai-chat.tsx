...
export default function AiChat() {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // If no relevant documents found, proceed with just the query
      if (!relevantDocs.length) {
        // Generate a response without context
        const answer = await generateCompletionWithContext("", input);
        setMessages((prev) => [
          ...prev,
          {
            role: "ai",
      ...
      setCurrentDocs(docContents);
      const answer = await generateCompletionWithContext(docContents, input);
          ...
          <>
            <div
              ...
              className={`mb-4 p-3 rounded ${
                message.role === "ai" ? "bg-blue-50" : "bg-green-50"
              }`}
            >
